...
const AddressProof = ({ handleNext, handlePrevious }) => {
  ...
  const handleFormSubmit = (data) => {
    ...
    // if (!photoURL) {
    //   notifyError("Please select Address Proof photo");
    //   return;
    // }
          ...
          <form onSubmit={handleSubmit(handleFormSubmit)}>
            ...
            <div className="grid lg:grid-cols-2 sm:grid-cols-1">
              <div className="flex flex-col justify-center">
                ...
                <ImageUpload {...{ photoURL, setOpenCrop, setPhotoURL, setFile, openCrop, label: 'Upload Address Proof', imagewidth, imageheight }} />


                <Error errorName={errors.AddressProofimg} />
                <h5 className='text-sm text-black'>(Aadhar Card / Driving Licence / Voter ID card / Light Bill, any one of these.)</h5>
              </div>
