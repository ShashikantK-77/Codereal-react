...
const DeleteModal = ({ id, ids, setIsCheck, category, title, useParamId,customer }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (location.pathname === "/clients") {
        ...
        // console.log("customercustomercustomer in delete model Client",Client);
        console.log("customercustomercustomer in delete model customer",customer);
        ...
        const updatedData = existingData.filter(
          (client) =>
            client.name !== customer.name || client.email !== customer.email
        );
        ...
        setClientListData(updatedData);

        notifySuccess(`Client [${customer.name}] Deleted Successfully` );
        setIsUpdate(true);
        setServiceId();
        closeModal();
        setIsSubmitting(false);
      ...
      if (location.pathname === "/masteraccounts") {
        console.log("in masteraccount delete");
        console.log("masteraccounts masteraccounts  masteraccounts in delete model Client",customer);
        // console.log("customercustomercustomer in delete model customer",customer);
        ...
        const updatedData = existingData.filter(
          (client) =>
            client.Account_Name !== customer.Account_Name || client.UniqueKey !== customer.UniqueKey
        );
        ...
        notifySuccess(`Mater [${customer.Account_Name}] Deleted Successfully` );
        setIsUpdate(true);
        setServiceId();
        closeModal();
        setIsSubmitting(false);
